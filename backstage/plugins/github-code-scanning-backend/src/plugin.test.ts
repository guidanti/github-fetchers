...
import { githubCodeScanningPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [githubCodeScanningPlugin],
    });

    await request(server).get('/api/github-code-scanning/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/github-code-scanning/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/github-code-scanning/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/github-code-scanning/todos/${createdTodoItem.id}`)
